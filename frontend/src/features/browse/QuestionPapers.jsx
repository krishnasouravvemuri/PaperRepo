...
import { listQuestionPapers, getQuestionPaperFileUrl } from "../../api/questionPapers.js";
...
export default function QuestionPapers() {
  ...
  const load = async (params) => {
    try {
      const data = await listQuestionPapers(params);
      setPapers(data || []);
    } catch (err) {
  ...
  const openFile = async (id, download) => {
    if (download && downloadingId) return; // ignore while a download is in flight
    try {
      if (download) {
        setDownloadingId(id);
        const { url, filename } = await getQuestionPaperFileUrl(id);
        await downloadFromUrl(url, filename);
      } else {
        const { url } = await getQuestionPaperFileUrl(id);
  ...
  return (
    <div className="flex flex-col gap-6 mt-10">
      <form onSubmit={onSearch} className="flex flex-col space-y-4">
        <label className="text-2xl font-semibold text-gray-200">Search for Question Papers:</label>
      ...
      {papers.length > 0 ? (
        <div className="mt-4 p-6 bg-[#1e1e2e] rounded-2xl shadow-2xl overflow-x-auto mb-4">
          ...
          <table className="min-w-full text-left border border-gray-700 rounded-lg">
            <thead className="bg-[#221c35] text-teal-400">
              <tr>
                {["Sl No", "Title", "Subject", "Code", "Slot", "Exam", "Semester", "Year", "Actions"].map(
                  (h) => (
              ...
              {papers.map((p, i) => (
                <tr key={p.question_paper_id} className="border-b border-gray-700 hover:bg-[#2a2235]">
                  ...
                  <td className="px-4 py-2 flex gap-2 justify-center">
                    <button
                      onClick={() => openFile(p.question_paper_id, false)}
                      className="px-2 py-1 bg-blue-500 rounded text-white hover:bg-blue-400"
                    >
                      View
                    </button>
                    <button
                      onClick={() => openFile(p.question_paper_id, true)}
                      disabled={downloadingId === p.question_paper_id}
                    ...
                    >
                      {downloadingId === p.question_paper_id ? (
                        <>
                          <Spinner /> Downloading…
                        </>
                      ) : (
        ...
        searched && <p className="mt-6 text-gray-400">No papers found.</p>
      )}
    </div>
  );
}
